...
# PROJECT    = "taketwo"
# CHUNK_SIZE = 64
# X_SCALE    = 256   # Q8 input scaling: x_int = round(x_float * 256)
...
#         default_bias_dtype=ng.int32,   # important: biases end up int32 in your map
...
#     if len(out_nodes) != 1:
#         raise RuntimeError(f"Expected 1 output, got {len(out_nodes)}: {list(outputs.keys())}")
#     top = out_nodes[0]

#     print("\nImported ONNX into NNgen")
#     print("Output node:", top)

#     # (2) Quantize (README Step 2 alternative)
...
#     # (4) Verify dataflow in software (ng.eval)
...
#     # (5) IMPORTANT: "Compile" / assign memory map by generating HW FIRST
#     # This is what triggers the Schedule Table and Default Memory Map.
#     m = ng.to_veriloggen(top, PROJECT)      # or ng.to_ipxact([...]) if you prefer
#     rtl = m.to_verilog()
...
#     # (5) Export packed parameters AFTER to_veriloggen/to_ipxact
#     param_data = ng.export_ndarray([top], CHUNK_SIZE)

#     if param_data.size == 0:
#         raise RuntimeError(
#             "export_ndarray returned empty even after to_veriloggen(). "
#             "Try export_ndarray(out_nodes, CHUNK_SIZE) or export_ndarray([top], 64) with to_ipxact()."
#         )

#     npy_path = os.path.join(OUT_DIR, f"{PROJECT}_params.npy")
#     np.save(npy_path, param_data)
#     print("Saved packed params (.npy):", npy_path)
#     print("param_data dtype:", param_data.dtype, "shape:", param_data.shape, "bytes:", param_data.size)

#     # Write raw bytes blob for easy RAM init
#     # export_ndarray commonly returns byte-values in an integer array; convert to uint8 before writing.
