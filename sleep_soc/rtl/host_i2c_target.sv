    ...
    if (!resetn) begin
        ...
        if (start_cond) begin
            ...
            case (state)
                    ...
                    if (scl_rise) begin
                        ...
                        if (bit_cnt == 3'd0) begin
                            ...
                            if (shift[7:1] == SLAVE_ADDR) begin
                                if (sda_sync && ptr_set) rd_addr_o <= reg_ptr;
                                state         <= ST_ADDR_ACK;
                                sda_drive_low <= 1'b1;
                                ack_seen_high <= 1'b0;
                            end else begin
                ...
                ST_ADDR_ACK: begin
                    if (scl_rise) ack_seen_high <= 1'b1;
                ...
                ST_RD_TX: begin
                    ...
                    if (scl_rise) begin
                        if (bit_cnt == 3'd0) begin
                            state <= ST_RD_ACK;
                            // Do NOT release SDA here — master samples during scl_rise.
                            // Hold data until scl_fall in RD_ACK so master sees the correct bit.
                        end else begin
                            bit_cnt <= bit_cnt - 3'd1;
                        end
                    end
                ...
                ST_RD_ACK: begin
                    if (scl_fall) begin
                        sda_drive_low <= 1'b0;  // release after master has sampled bit 0
                    end
                    if (scl_rise) begin
                        if (!sda_sync) begin
                            reg_ptr   <= reg_ptr + 8'd1;
